## Previous task quarantined — moving on

Task `{{quarantined_task_id}}` was quarantined for the following reason:

> {{quarantine_reason}}

The branch `{{quarantined_branch}}` is left intact (not deleted). Quarantine report at `.local/ai-supervisor/quarantine/{{quarantined_task_id}}.md`.

**Do not continue working on that branch.** Move to the next task below.
...
5. `git checkout -b {{task_branch_hint}}`.
{{/if}}
